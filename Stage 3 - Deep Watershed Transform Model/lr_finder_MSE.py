...
# ==========================================
# DEFINE WRAPPERS (Crucial for LRFinder)
# ==========================================
...
class WTN_LR_Wrapper(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        inputs, direction, energy = self.ds[idx]
        # Normalise Energy
        normalized_energy = energy.unsqueeze(0).float() / 15.0
        return direction, normalized_energy
...
if __name__ == "__main__":
    ...
    # A. Setup Model
    wtn_model = dwt_network.WatershedTransformNetwork(out_classes=1).to(DEVICE)
    ...
    3e-4 (rounded from 2.48e-4)
    
    --- WTN Suggestion: 0.000359 ---
    LR suggestion: steepest gradient
    Suggested LR: 3.59E-04
    4e-4 (rounded from 3.59e-4)
    '''