...
class Dataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # 2. Convert to tensor and immediately threshold to 0 and 1
...
def train(epochs, batch_size, lr, weights_list):
    ...
    model = smp.PSPNet(
        encoder_name="resnet18",  # Based on report
        encoder_weights="imagenet",  # imagenet
        classes=2,  # Set to 2 to match Unet
        activation=None,
    ).to(DEVICE)
    ...
    scheduler = optim.lr_scheduler.ReduceLROnPlateau(
        ...
        patience=2 # Wait 2 epoch of no improvement before cutting
    )
    ...
    for epoch in range(epochs):
        ...
        accumulation_steps = 4  # 4 steps * Batch Size 4 = Virtual Batch Size 16
        optimizer.zero_grad()
...
if __name__ == "__main__":
    ...
    plt.suptitle('PSPNet Testing', fontsize=16)
    plt.show()

    '''
    #Debugging#:
    Learning Rate: 1.71E-04 (Recommended by PyTorch LR Finder).
...
#     BATCH_SIZE = 4
#
#     temp_model = smp.PSPNet(
#     encoder_name="resnet18",
#     encoder_weights="imagenet",
#     classes=2, # Set to 2 to match your binary task
#     activation=None,
# )
...
'''
    ...
    Mechanism
    - Executes and miniature 'mock' training session (usually about 70-100 iterations)
