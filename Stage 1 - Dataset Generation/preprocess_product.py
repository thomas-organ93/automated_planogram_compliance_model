...
def get_random_distractor(coco_obj, exclude_cat_id, img_dir):
    ...
    # Retry loop in case pick a bad/empty image
    for _ in range(5):
...
def get_random_crop_from_category(coco_obj, target_cat_id, img_dir):
    ...
    # Retry loop to ensure a valid crop
    for _ in range(5):
        ...
        # Get annotations for this image that match target category
...
def paste_single_product_to_shelf(product_img, shelf_img, blur_foreground=False):
    """
    Blends a 448x448 product into a 1024x512 shelf.
    Returns a 448x448 shelf crop with the product smoothly pasted.
    """
    # Setup dimensions
    ...
    # This simulates real camera focus and prevents 'cut-and-paste' artifacts
    kernel = np.ones((3, 3), np.uint8)
    # Erode the mask by 1-2 pixels to "tuck" the edges under the product
    ...
    if blur_foreground:
        # Blur the color data of the product
        product_img = cv2.GaussianBlur(product_img, (7, 7), 3.0)
...
def paste_product_to_shelf(shelf_rgb, shelf_mask, product_rgb, product_mask, x_pos, y_pos):
    ...
    Uses a 'Global Moat' strategy to guarantee separation, even at the edges of the paste area.
    """
    ...
    # Create a temporary full-size canvas for THIS object
    # This prevents the "Crop Edge" bug where dilation stops at the box boundary
